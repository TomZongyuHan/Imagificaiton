...
class ImageTransformer:
    ...
    def __init__(self, feature_extractor='tsne', pixels=100,
        ...
        if isinstance(feature_extractor, str):          #isinstance means if the feature_extractor is str
            fe = feature_extractor.casefold()           #casfold() converts all characters of the string into lowercase letters and returns a new string.
            if fe == 'tsne'.casefold():
                fe = TSNE(
            ...
            self._fe = fe                                       #Check if the feature_extractor object has the "fit_transform" property:
    ...
    def fit(self, X, y=None, plot=False):
        ...
        x_new = self._fe.fit_transform(X.T)         #Fit X.T into an embedded space and return that transformed output.
        ...
        self._xrot = np.dot(mbr_rot, x_new.T).T         #np.dot(a,b) : dot product of two arrays a and b
    ...
    def pixels(self, pixels):
        ...
        if hasattr(self, '_coords'):      #The hasattr() function returns True if the specified object has the specified attribute, otherwise False.
            self._calculate_coords()
    ...
    def transform(self, X, format='rgb', empty_value=0):
        ...
        img_coords = pd.DataFrame(np.vstack((
            self._coords.T,
            X
        )).T).groupby([0, 1], as_index=False).mean()     #numpy.vstack() function is used to stack arrays vertically (row-wise) to make a single array.
    ...
    def _mat_to_rgb(mat):
        ...
        Returns:
            ...
            RGB channels.
        """
        return np.repeat(mat[:, :, np.newaxis], 3, axis=2)       #numpy.repeat(a, repeats, axis=None/int)


class LogScaler:
    """Log normalize and scale data

    Log normalization and scaling procedure in the
